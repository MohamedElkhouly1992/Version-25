...
def apply_predicted_zone_deadband_to_loads(
    ...
    if not predicted_zone_deadband_enabled(cfg):
        ...
        return float(q_cool_kw), float(q_heat_kw), info
    q_cool = float(q_cool_kw)
    q_heat = float(q_heat_kw)
...
def _build_daily_export_from_timestep_data(timestep_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Daily-friendly aliases generated from period sums.
    if "energy_kwh_period" in daily.columns:
        daily["energy_kwh_day"] = daily["energy_kwh_period"]
    if "co2_kg_period" in daily.columns:
        daily["co2_kg_day"] = daily["co2_kg_period"]
    if "cost_usd_period" in daily.columns:
        daily["cost_usd_day"] = daily["cost_usd_period"]
    ...
    if comp:
        daily["daily_component_balance_kwh"] = daily[comp].sum(axis=1)
        if "energy_kwh_period" in daily.columns:
            daily["daily_balance_error_kwh"] = daily["energy_kwh_period"] - daily["daily_component_balance_kwh"]
